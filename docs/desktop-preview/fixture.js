...
// V0.24 UI fixture: isolated browser storage, never production accounts.
(()=>{const key='qanteak.v024.preview.modules';let rows=JSON.parse(localStorage.getItem(key)||'[]'),history=JSON.parse(localStorage.getItem(key+'.history')||'{}'),prefs={};const persist=()=>{localStorage.setItem(key,JSON.stringify(rows));localStorage.setItem(key+'.history',JSON.stringify(history))};window.qanteakDesktop.workspaceModules=async(w,a,d={})=>{if(a==='list')return rows.filter(r=>!r.deleted&&(!d.kind||r.kind===d.kind));if(a==='inbox')return{items:[],preferences:prefs};if(a==='preferences'){prefs=d;return d}if(a==='save'){const old=rows.find(r=>r.id===d.id);if(old&&old.version!==d.version)throw Error('Version conflict');const r={...d,id:d.id||crypto.randomUUID(),version:(old?.version||0)+1,created_by:'preview-user',updated_at:new Date().toISOString(),data:structuredClone(d.data||{})};rows=rows.filter(x=>x.id!==r.id);rows.unshift(r);history[r.id]=[{version:r.version,payload:structuredClone(r),created_at:r.updated_at},...(history[r.id]||[])];persist();return r}if(a==='blocks.save'){const r=rows.find(x=>x.id===d.id);let blocks=r.data.blocks||[];for(const p of d.patches){blocks=blocks.filter(b=>b.id!==p.id);if(!p.remove)blocks.push(p.value)}return window.qanteakDesktop.workspaceModules(w,'save',{...r,data:{...r.data,blocks}})}if(a==='runs')return[];if(a==='history')return history[d.id]||[];if(a==='comments')return[];if(a==='comment')return{};if(a==='delete'){rows=rows.filter(r=>r.id!==d.id);persist();return{}}if(a==='share')throw Error('Public sharing is disabled in the synthetic preview.');return{}};window.qanteakDesktop.workspaceFeedback=async()=>[];window.qanteakDesktop.workspaceAsk=async()=>({message:'Synthetic preview: workspace AI is connected in the installed app. This preview does not call an AI provider.',sources:[],actions:[]});})();
