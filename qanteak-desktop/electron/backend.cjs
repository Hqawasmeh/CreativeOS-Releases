...
const cloudDelay=ms=>new Promise(resolve=>setTimeout(resolve,ms));
...
async function startRealtime(workspaceId,onChange){stopRealtime();const c=loadConfig(),s=await validSession();if(!configured()||!s?.access_token||!workspaceId)return{ok:false};realtimeClient=createClient(c.url,c.key,{auth:{persistSession:false,autoRefreshToken:false},global:{headers:{Authorization:`Bearer ${s.access_token}`}}});await realtimeClient.realtime.setAuth(s.access_token);realtimeChannel=realtimeClient.channel(`workspace:${workspaceId}`)
 ...
 .on('postgres_changes',{event:'*',schema:'public',table:'q23_messages'},payload=>onChange?.({type:'chat',new:payload.new}))
 .on('postgres_changes',{event:'*',schema:'public',table:'q24_records',filter:`workspace_id=eq.${workspaceId}`},payload=>onChange?.({type:'modules',new:payload.new}))
 .on('postgres_changes',{event:'INSERT',schema:'public',table:'q24_inbox',filter:`workspace_id=eq.${workspaceId}`},payload=>onChange?.({type:'inbox',new:payload.new}))
...
module.exports.workspaceModules=(workspaceId,action,data={})=>action==='blocks.save'?rpc('q24_document',{p_workspace_id:workspaceId,p_data:data}):action==='runs'?rpc('q24_runs',{p_workspace_id:workspaceId,p_record_id:data.id}):rpc('q24_workspace',{p_workspace_id:workspaceId,p_action:action,p_data:data});
module.exports.chatRequest=(workspaceId,action,data={})=>rpc('q24_chat',{p_workspace_id:workspaceId,p_action:action,p_data:data});
module.exports.chatUpload=async(roomId,name,base64)=>{if(!/^[a-f0-9-]{36}$/.test(roomId)||typeof base64!=='string'||base64.length>14000000)throw Error('Invalid attachment');const c=loadConfig(),s=await validSession();if(!s)throw Error('Sign in first');const bytes=Buffer.from(base64,'base64');if(bytes.length>10485760)throw Error('Maximum attachment size is 10 MB');const path=roomId+'/'+require('node:crypto').randomUUID()+'/'+String(name).replace(/[^a-zA-Z0-9._-]/g,'_').slice(0,120);await jsonFetch(`${c.url}/storage/v1/object/qanteak-chat/${path}`,{method:'POST',headers:{apikey:c.key,Authorization:`Bearer ${s.access_token}`,'Content-Type':'application/octet-stream'},body:bytes});return{path,name:String(name).slice(0,120),size:bytes.length}};
module.exports.chatDownload=async(path)=>{if(typeof path!=='string'||path.includes('..'))throw Error('Invalid path');const c=loadConfig(),s=await validSession();if(!s)throw Error('Sign in first');const r=await jsonFetch(`${c.url}/storage/v1/object/sign/qanteak-chat/${path}`,{method:'POST',headers:{apikey:c.key,Authorization:`Bearer ${s.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({expiresIn:60})});return c.url+'/storage/v1'+r.signedURL};
module.exports.workspaceFeedback=(w,id)=>rpc('q24_feedback',{p_workspace_id:w,p_record_id:id});
module.exports.workspaceAsk=async(w,message)=>{const c=loadConfig(),s=await validSession();if(!s)throw Error('Sign in first');const context=await rpc('q24_ai_context',{p_workspace_id:w,p_query:String(message).slice(0,4000)});context.sources=context.sources.map((v,i)=>({...v,source:'S'+(i+1)}));const answer=await jsonFetch(`${c.url}/functions/v1/qanteak-ai`,{method:'POST',headers:{apikey:c.key,Authorization:`Bearer ${s.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({message:String(message).slice(0,4000),mode:'ask',context})},60000);return{...answer,sources:context.sources.map(({content,...v})=>v)}};
module.exports.moduleUpload=async(recordId,name,base64)=>{if(!/^[a-f0-9-]{36}$/.test(recordId)||typeof base64!=='string'||base64.length>14000000)throw Error('Invalid attachment');const c=loadConfig(),s=await validSession();if(!s)throw Error('Sign in first');const bytes=Buffer.from(base64,'base64');if(bytes.length>10485760)throw Error('Maximum attachment size is 10 MB');const path=recordId+'/'+require('node:crypto').randomUUID()+'/'+String(name).replace(/[^a-zA-Z0-9._-]/g,'_').slice(0,120);await jsonFetch(`${c.url}/storage/v1/object/qanteak-modules/${path}`,{method:'POST',headers:{apikey:c.key,Authorization:`Bearer ${s.access_token}`,'Content-Type':'application/octet-stream'},body:bytes});return{path,name:String(name).slice(0,120),size:bytes.length}};
module.exports.moduleDownload=async(path)=>{if(typeof path!=='string'||path.includes('..'))throw Error('Invalid path');const c=loadConfig(),s=await validSession();if(!s)throw Error('Sign in first');const r=await jsonFetch(`${c.url}/storage/v1/object/sign/qanteak-modules/${path}`,{method:'POST',headers:{apikey:c.key,Authorization:`Bearer ${s.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({expiresIn:60})});return c.url+'/storage/v1'+r.signedURL};
