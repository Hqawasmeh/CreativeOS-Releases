...
begin
 ...
 perform public.q23_collaboration(w,'reminder.save',jsonb_build_object('title','QA reminder','due_at',now()+interval '1 hour'));
 ...
 begin perform * from public.q23_messages;raise exception 'Direct table access unexpectedly allowed';exception when insufficient_privilege then null;end;
end $$;
...
rollback;
select 'PASS: membership, private chat isolation, direct/message deduplication, attendance, personal reminders/layouts, admin and anonymous boundaries; all test writes rolled back' as result;
